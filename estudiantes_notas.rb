...
class Estudiante
    ...
    attr_reader :notas, :nombre, :curso
...
class Curso
    ...
    def initialize(nombre, cantidad_estudiantes = 10)
      @nombre = nombre
      @estudiantes = []
    ...
    def promedio_general_curso
        total_promedios = @estudiantes.sum { |e| e.promedio_general } # Suma de promedios generales
  ...
  # Método para mostrar el ranking del curso basado en los promedios generales
    def ranking_estudiantes
        ...
        estudiantes_ordenados = @estudiantes.sort_by { |e| e.promedio_general }.reverse
  ...
  # Método para calcular el promedio por asignatura en el curso
    def promedio_por_asignatura
        ...
        total_promedio_asignatura = @estudiantes.sum { |e| e.promedios[asignatura.to_sym] }
        promedio_asignatura = (total_promedio_asignatura / @estudiantes.length).round(1)
            puts "Promedio de #{asignatura.capitalize} en el curso: #{promedio_asignatura}"
        end
    end
...
#configuracion para crear estudiantes y asignarles notas
#numero_estudiantes 10 numeros de estudiantes a generar
...
#usar los métodos definidos: Ranking del curso, ranking por asignatura del curso, informe de notas de un estudiante
